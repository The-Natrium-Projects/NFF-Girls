...
// Ported from hmag
@OnlyIn(Dist.CLIENT)
public class NFFGirlsHmagEnderExecutorRenderer extends MobRenderer<HmagEnderExecutorEntity, NFFGirlsHmagEnderExecutorModel<HmagEnderExecutorEntity>>
{
	...
	public NFFGirlsHmagEnderExecutorRenderer(EntityRendererProvider.Context context)
	{
		super(context, new NFFGirlsHmagEnderExecutorModel<>(context.bakeLayer(ModModelLayers.ENDER_EXECUTOR)), 0.5F);
		this.addLayer(new NFFGirlsHmagEnderExecutorEyesLayer<>(this));
		this.addLayer(new ItemInHandLayer2<>(this, -1));
		this.addLayer(new NFFGirlsHmagEnderExecutorCarriedBlockLayer(this));
	}
	...
	public void render(HmagEnderExecutorEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight)
	{
		...
		NFFGirlsHmagEnderExecutorModel<HmagEnderExecutorEntity> endermanmodel = this.getModel();
