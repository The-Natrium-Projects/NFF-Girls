...
public class HmagAlrauneEntity extends AlrauneEntity implements INFFGirlsTamed {
	...
	{
		this.shootSeed(target, () -> new NFFGirlsHmagAlrauneSeedEntity.PoisonSeed(level, this), distanceFactor);
	}
	
	public void shootSeed(LivingEntity target, Supplier<? extends NFFGirlsHmagAlrauneSeedEntity> shotSupplier, float distanceFactor)
	{
		int c = this.getRandom().nextInt(3) + 1;

		for (int i = 0; i < c; ++i)
		{
			NFFGirlsHmagAlrauneSeedEntity shot = shotSupplier.get();
...
*/
	...
	protected static class ShootHealingGoal extends NFFShootProjectileGoal
	{
		...
		{
			((HmagAlrauneEntity)mob).shootSeed(target, () -> new NFFGirlsHmagAlrauneSeedEntity.HealingSeed(mob.asMob().level, mob), velocity);
		}
		...
		protected LivingEntity updateTarget() {
			...
			if (!owner.isEmpty())
				return owner.get(0);			
			List<LivingEntity> allies = visible.stream().filter((LivingEntity living) -> NFFGirlsHmagAlrauneSeedEntity.HealingSeed.canAffect(mob, living))
