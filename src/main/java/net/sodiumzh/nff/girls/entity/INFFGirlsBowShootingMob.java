...
public interface INFFGirlsBowShootingMob extends INFFGirlsTamed, INFFGirlsBowShootingMobUtils
{
	...
	public default AbstractArrow shoot(LivingEntity pTarget, float pVelocity) {
		...
		boolean canPickUp = EnchantmentHelper.getItemEnchantmentLevel(Enchantments.INFINITY_ARROWS, this.getAdditionalInventory().orElseThrow().getItem(4)) <= 0
	...
	{
		if (EnchantmentHelper.getItemEnchantmentLevel(Enchantments.INFINITY_ARROWS, this.getAdditionalInventory().orElseThrow().getItem(4)) <= 0
				|| this.getAdditionalInventory().orElseThrow().getItem(8).is(Items.TIPPED_ARROW) || this.getAdditionalInventory().orElseThrow().getItem(8).is(Items.SPECTRAL_ARROW))
