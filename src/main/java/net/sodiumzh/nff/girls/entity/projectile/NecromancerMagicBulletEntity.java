...
import net.sodiumzh.nff.girls.entity.tamingprocess.HmagNecroticReaperTamingProcess;
...
public class NecromancerMagicBulletEntity extends MagicBulletEntity
{
	...
	public NecromancerMagicBulletEntity(Level worldIn, LivingEntity shooter, double accelX, double accelY, double accelZ)
	{
		...
		NFUReflectionStatics.forceSet(this, Entity.class, "f_19847_", NFFGirlsEntityTypes.NECROMANCER_MAGIC_BULLET.get());	// Entity.type
		this.setVariant(1);
	}
	...
	protected void blast(Entity ignore)
	{
		...
		NFUEntityStatics.sendParticlesToEntity(this, ParticleTypes.EXPLOSION, 0, 0, 2, 0);
			...
			if (!(living instanceof ArmorStand))
			{
				...
				else if (living instanceof HmagNecroticReaperEntity nr)
				{
					if (this.getOwner() instanceof Player player && nr.getOwnerUUID().equals(player.getUUID()))
					{
						...
						{
							nr.heal(6);
							NFUEntityStatics.addEffectSafe(nr, MobEffects.DAMAGE_BOOST, 300, 1);
						} else
						{
							nr.heal(4);
						}
						NFUEntityStatics.sendGlintParticlesToLivingDefault(nr);
					}
					shouldAddWither = false;
				}
				if (shouldAddWither)
				{
					NFUEntityStatics.addEffectSafe(living, MobEffects.WITHER, 200, hasNecromancerHat ? 4 : 3);
			...
			if (!(living instanceof ArmorStand))
			{
				...
				else if (living instanceof HmagNecroticReaperEntity nr)
				{
					if (this.getOwner() instanceof Player player && nr.getOwnerUUID().equals(player.getUUID()))
					{
						...
						{
							nr.heal(3);
							NFUEntityStatics.addEffectSafe(nr, MobEffects.DAMAGE_BOOST, 200, 1);
						} else
						{
							nr.heal(2);
						}
				...
				if (shouldAddWither)
				{
					NFUEntityStatics.addEffectSafe(living, MobEffects.WITHER, 200, hasNecromancerHat ? 3 : 2);
					if (this.getOwner() instanceof LivingEntity ownerLiving)
					{
						living.setLastHurtByMob(ownerLiving);
					}
