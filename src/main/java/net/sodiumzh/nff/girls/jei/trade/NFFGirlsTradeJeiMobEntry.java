...
public class NFFGirlsTradeJeiMobEntry implements IRecipeCategoryExtension {
    ...
    public void drawInfo(int recipeWidth, int recipeHeight, @NotNull PoseStack guiGraphics, double mouseX, double mouseY) {
        ...
        for (int i = 0; i < merchantLevels.size(); ++i) {
            NFUGUIStatics.setActiveTexture(TEXTURE_PATH);
            NFUGUIStatics.drawSprite(guiGraphics, new GuiPos(126, y0 + i * 22), 0,
                new GuiPos(0, 120), new GuiPos(20, 20), TEXTURE_SIZE);
            NFUGUIStatics.drawSprite(guiGraphics, new GuiPos(90, y0 + i * 22), 0,
                new GuiPos(22, 120), new GuiPos(18, 18), TEXTURE_SIZE);
            NFUGUIStatics.drawSprite(guiGraphics, new GuiPos(109, y0 + i * 22), 0,
                new GuiPos(22, 120), new GuiPos(18, 18), TEXTURE_SIZE);
            NFUGUIStatics.drawSprite(guiGraphics, new GuiPos(146, y0 + i * 22), 0,
                new GuiPos(22, 120), new GuiPos(18, 18), TEXTURE_SIZE);
        }
        // Draw level tips
        for (int i = 0; i < merchantLevels.size(); ++i) {
            Minecraft.getInstance().font.draw(guiGraphics, "lv. " + levelRequirements[merchantLevels.get(i) - 1], 66, y0 + 5 + i * 22, 8);

        };
        // Draw mob name
        Minecraft.getInstance().font.draw(guiGraphics, entityAndEntries.getA().map(Entity::getName)
            .orElseGet(() -> NFUInfoStatics.createText("")).getVisualOrderText(), 5, 5, 8);
    }
