...
public class CombatCommandingWandItem extends NFUItem {

	private static final String KEY_SETTING_MOB_IDENTIFIER = "settingIdentifier";

	public CombatCommandingWandItem(Item.Properties pProperties) {
		super(pProperties);
	}
	...
	public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand usedHand) {
		if (!level.isClientSide()) {
			HitResult res = NFULevelStatics.eyeTrace(player, 32d).orElse(null);
			...
			// Not targeting entity, reset
				...
				if (player.getItemInHand(usedHand).hasTag() && player.getItemInHand(usedHand).getTag().hasUUID(KEY_SETTING_MOB_IDENTIFIER)) {
					...
					NFUInfoStatics.printMessageTranslatable(player, "info.nffgirls.item.combat_set_target_reset");
	...
	private InteractionResult doServerInteractLivingEntity(Player player, LivingEntity target, InteractionHand hand) {
		if (player.isShiftKeyDown()) {
			return INFFGirlsTamed.get(target).filter(t -> t.isOwnedBy(player)).map(tamed -> {
				...
				NFUInfoStatics.printMessageTranslatable(player, "info.nffgirls.item.combat_stop_attacking", target.getName().getString());
				...
				if (set.get())
					NFUInfoStatics.printMessageTranslatable(player, "info.nffgirls.item.combat_set_target_all", target.getName().getString());
				return InteractionResult.sidedSuccess(player.level().isClientSide());
			});
		}
			...
			if (player.level() instanceof ServerLevel sl) {
				...
				if (trySetTarget(player, attacker, target)) {
					NFUInfoStatics.printMessageTranslatable(player, "info.nffgirls.item.combat_set_target", attacker.getName().getString(), target.getName().getString());
				} else {
					NFUInfoStatics.printMessageTranslatable(player, "info.nffgirls.item.combat_set_target_failed");
				}
